...
Status per 31 Agustus 2026. Aset yang **sudah ada** ada di `assets/img/{logo,graphics,bg}/` (lihat `docs/BRAND-AUDIT.md`) — tidak diulang di sini. Daftar di bawah adalah yang **belum ada** dan dibutuhkan untuk build.

## 🔴 Blocking untuk Tahap 1 (hero Beranda)

| Aset | Dimensi / format | Dipakai di | Catatan |
|---|---|---|---|
| Foto/KV hero Beranda | 1920×1080 min, WebP + JPEG fallback, crop aman untuk 375px & 1440px | index.html | **Jangan pakai foto dari deck/poster sample** — status hak pakai tidak jelas (§1.6, §6.5 no.10). Kalau foto belum ada saat Tahap 1, hero dibangun dengan latar `--gradient-violet` + sistem grafis (loop/elemen gerak) sebagai fallback, tanpa foto. |
...
| Font pengganti final | approval visual Fredoka+Barlow | Paris2024 & Comba Test tidak bisa dipakai produksi (lisensi) — Fredoka dipilih sebagai kandidat, perlu sign-off desainer sebelum dikunci |
